...
class KnowledgeSearchTool(BaseTool):
    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            print(f"[KnowledgeSearch] Searching: '{query}' (top_k={top_k})")
            
            if self._document_scope:
                print(f"[KnowledgeSearch] Scope: {len(self._document_scope)} documents")
            
            # Perform scoped search
            results = self._knowledge_store.search_scoped(
                query=query,
                top_k=top_k,
                document_ids=self._document_scope
            )
            ...
            return ToolResult(
                ...
                metadata={
                    ...
                    "document_ids": list({r.get("document_id") for r in results if r.get("document_id")}),
                }
            )
            
        except Exception as e:
            print(f"[KnowledgeSearch] Error: {e}")
            return ToolResult(
                success=False,
